...
/--
Typed factorization object for the constructive-to-endpoint route through the
vacuum-gap side.
-/
structure YMNativeConstructiveEndpointFactorization
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  law_object : YMNativeCrossHeartLawObject RC RD RE
  composite_endpoint_map : YMSharpLocalState -> RE.endpoint_object.exact_local_net_endpoint
  ...
  composite_image_definition :
    composite_endpoint_map chosen_constructive_state = composite_endpoint_image
  direct_image_definition :
    law_object.compatibility.constructive_to_endpoint chosen_constructive_bounded_base =
      direct_endpoint_image
  factorization_agrees :
    composite_endpoint_image = direct_endpoint_image

def YangMillsNativeConstructiveEndpointFactorizationData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeConstructiveEndpointFactorization RC RD RE := by
  let O :=
  ...
  let f :
    ...
    fun s =>
      O.compatibility.vacuum_gap_to_endpoint
        (O.compatibility.constructive_state_to_vacuum_gap s).down
  refine
    { law_object := O
      composite_endpoint_map := f
      chosen_constructive_state := A.constructive.interface.chosen_sharp_local_state
      composite_endpoint_image := f A.constructive.interface.chosen_sharp_local_state
      chosen_constructive_bounded_base := A.constructive.bounded_base_witness
      direct_endpoint_image :=
        O.compatibility.constructive_to_endpoint A.constructive.bounded_base_witness
      composite_image_definition := rfl
      direct_image_definition := rfl
      factorization_agrees := rfl }

theorem YangMillsNativeConstructiveEndpointFactorizationWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  (etest : (YangMillsEndpointSemanticBundleData RE hE hP).test_function_space)
  (efield : (YangMillsEndpointSemanticBundleData RE hE hP).field_family) :
  Nonempty (YMNativeConstructiveEndpointFactorization RC RD RE) /\
