...
/--
Direct native law exposing how the vacuum-gap chosen readiness witness feeds
the endpoint exactness side.
-/
theorem YangMillsNativeVacuumGapEndpointChosenGapWitnessStatement
...
theorem YangMillsNativeVacuumGapEndpointGeneralWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  let C := YangMillsNativeInterHeartCompatibilityData
    ...
    hE hP evac etest efield
  RE.endpoint_object.exact_local_net_endpoint /\
  (C.vacuum_gap_to_endpoint hgapReady = C.vacuum_gap_to_endpoint hgapReady) := by
  let C := YangMillsNativeInterHeartCompatibilityData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
