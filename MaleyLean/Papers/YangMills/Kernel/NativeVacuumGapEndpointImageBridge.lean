...
/--
Typed refinement recording the chosen vacuum-gap readiness witness together
with its endpoint image.
-/
structure YMNativeVacuumGapEndpointImageBridge
  ...
  endpoint_image : RE.endpoint_object.exact_local_net_endpoint
  image_bridge_compatibility :
    law_object.compatibility.vacuum_gap_to_endpoint chosen_vacuum_gap_ready =
      endpoint_image

def YangMillsNativeVacuumGapEndpointImageBridgeData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeVacuumGapEndpointImageBridge RC RD RE := by
  let O :=
...
theorem YangMillsNativeVacuumGapEndpointImageBridgeWitnessStatement
  ...
  let B :=
    YangMillsNativeVacuumGapEndpointImageBridgeData
      ...
      hww vtm vobs vrsec vcorr vgapf vhgap
      hE hP evac etest efield
  exact And.intro ⟨B⟩ <| And.intro ⟨B.law_object⟩ B.image_bridge_compatibility
