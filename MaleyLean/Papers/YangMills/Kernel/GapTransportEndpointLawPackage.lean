...
/--
Structured package for the vacuum-gap-to-endpoint bridge law.
-/
structure YMGapTransportEndpointLawPackage
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  law : YMGapTransportFeedsEndpointLaw RD RE
  ...
  yields_exact_endpoint :
    RD.transport_package.positive_gap_exhibited ->
      RE.endpoint_object.exact_local_net_endpoint

def YangMillsGapTransportEndpointLawPackageData
  ...
  YMGapTransportEndpointLawPackage RD RE := by
  ...
  refine
    { law := hO.gap_transport_feeds_endpoint
      yields_vacuum_correlations := by
        intro hgap
        exact (hO.gap_transport_feeds_endpoint hgap).1
      yields_exact_endpoint := by
        intro hgap
        exact (hO.gap_transport_feeds_endpoint hgap).2 }

theorem YangMillsGapTransportEndpointLawPackageExistsStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  (etest : (YangMillsEndpointSemanticBundleData RE hE hP).test_function_space)
  (efield : (YangMillsEndpointSemanticBundleData RE hE hP).field_family) :
  Nonempty (YMGapTransportEndpointLawPackage RD RE) := by
  exact ⟨YangMillsGapTransportEndpointLawPackageData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield⟩

theorem YangMillsGapTransportEndpointLawPackageWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  have hPck := YangMillsGapTransportEndpointLawPackageExistsStatement
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
