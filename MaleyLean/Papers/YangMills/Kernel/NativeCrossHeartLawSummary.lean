...
/--
Unified kernel-level summary bundling the three direct chosen-site native
cross-heart laws.
-/
theorem YangMillsNativeCrossHeartLawSummaryStatement
  ...
  True := by
  have hCV :=
    YangMillsNativeConstructiveVacuumGapChosenStateWitnessStatement
      RC RD RE
  ...
  have hVE :=
    YangMillsNativeVacuumGapEndpointChosenGapWitnessStatement
      ...
      hE hP evac etest efield
  have hCE :=
    YangMillsNativeConstructiveEndpointChosenWitnessStatement
      RC RD RE
      htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
      hww vtm vobs vrsec vcorr vgapf vhgap
      hE hP evac etest efield
  rcases hCV with ⟨hC, hGap, hLawCV⟩
  rcases hVE with ⟨_, hExactVE, hLawVE⟩
  rcases hCE with ⟨_, hExactCE, hLawCE⟩
  exact ⟨hC, hGap, hExactVE, True.intro⟩
