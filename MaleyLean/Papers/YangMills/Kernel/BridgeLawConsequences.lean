...
/--
Named higher-level consequences extracted from the bridge-law assembly.
-/
structure YMBridgeLawConsequences
...
def YangMillsBridgeLawConsequencesData
  ...
  YMBridgeLawConsequences RC RD RE := by
  let A := YangMillsBridgeLawAssemblyData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
  refine
    { yields_os_transport := A.bounded_base_transport.yields_os_transport
      records_transport_origin := A.bounded_base_transport.records_transport_origin
      yields_vacuum_correlations := A.gap_transport_endpoint.yields_vacuum_correlations
      yields_gap_exact_endpoint := A.gap_transport_endpoint.yields_exact_endpoint
      yields_constructive_smearing := A.constructive_endpoint.yields_smearing
      yields_constructive_exact_endpoint := A.constructive_endpoint.yields_exact_endpoint }

theorem YangMillsBridgeLawConsequencesExistsStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  (etest : (YangMillsEndpointSemanticBundleData RE hE hP).test_function_space)
  (efield : (YangMillsEndpointSemanticBundleData RE hE hP).field_family) :
  Nonempty (YMBridgeLawConsequences RC RD RE) := by
  exact ⟨YangMillsBridgeLawConsequencesData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield⟩

theorem YangMillsBridgeLawConsequencesWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  have hC := YangMillsBridgeLawConsequencesExistsStatement
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
  rcases hC with ⟨C⟩
  exact And.intro ⟨C⟩ <| And.intro C.yields_os_transport <|
    And.intro C.yields_vacuum_correlations C.yields_constructive_exact_endpoint
