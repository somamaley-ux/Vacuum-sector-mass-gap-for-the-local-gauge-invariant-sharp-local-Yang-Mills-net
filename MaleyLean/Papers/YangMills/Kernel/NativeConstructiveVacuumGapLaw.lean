...
theorem YangMillsNativeConstructiveVacuumGapGeneralWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  let C := YangMillsNativeInterHeartCompatibilityData
    ...
    hE hP evac etest efield
  RD.reconstruction_package.minkowski_gap_ready /\
  (C.constructive_to_vacuum_gap hbase = C.constructive_to_vacuum_gap hbase) := by
  let C := YangMillsNativeInterHeartCompatibilityData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
  exact And.intro
    (C.constructive_to_vacuum_gap hbase)
    rfl
