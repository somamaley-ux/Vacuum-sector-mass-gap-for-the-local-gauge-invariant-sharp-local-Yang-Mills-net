...
/--
Typed bridge-to-theorem interface packaging the heart-bridge object together
with the landing maps into the named main-theorem parts.
-/
structure YMTheoremBridgeInterface
    (S : YMLoadBearingSpine)
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  heart_bridge : YMHeartBridgeInterface RC RD RE
  partC_landing :
...
def YangMillsTheoremBridgeInterfaceData
  (S : YMLoadBearingSpine)
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  (hEalign :
    RE.endpoint_object.exact_local_net_endpoint ->
      S.theorem_parts.endpoint_exact_object) :
  YMTheoremBridgeInterface S RC RD RE := by
  refine
    { heart_bridge :=
        YangMillsHeartBridgeInterfaceData
          RC RD RE
          htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
          hww vtm vobs vrsec vcorr vgapf vhgap
          hE hP evac etest efield
      partC_landing :=
        YangMillsHeartBridgeConstructiveAlignsWithPartCStatement S RC hCalign
      partD_landing :=
        YangMillsHeartBridgeVacuumGapAlignsWithPartDStatement S RD hDalign
      endpoint_landing :=
        YangMillsHeartBridgeEndpointAlignsWithEndpointPartStatement S RE hEalign }

theorem YangMillsTheoremBridgeInterfaceExistsStatement
  (S : YMLoadBearingSpine)
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  (hEalign :
    RE.endpoint_object.exact_local_net_endpoint ->
      S.theorem_parts.endpoint_exact_object) :
  Nonempty (YMTheoremBridgeInterface S RC RD RE) := by
  exact ⟨YangMillsTheoremBridgeInterfaceData
    S RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
...
theorem YangMillsTheoremBridgeInterfaceWitnessStatement
  (S : YMLoadBearingSpine)
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  (hEalign :
    RE.endpoint_object.exact_local_net_endpoint ->
      S.theorem_parts.endpoint_exact_object) :
  Nonempty (YMTheoremBridgeInterface S RC RD RE) /\
  (RC.sharp_local_package.sharp_local_state.extends_bounded_base ->
    S.theorem_parts.partC_sharp_local_construction) /\
  (RD.transport_package.os_transport_ready ->
    S.theorem_parts.partD_vacuum_gap) /\
  ...
  have hI := YangMillsTheoremBridgeInterfaceExistsStatement
    S RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
    hCalign hDalign hEalign
  rcases hI with ⟨I⟩
  exact And.intro ⟨I⟩ <| And.intro I.partC_landing <|
    And.intro I.partD_landing I.endpoint_landing
